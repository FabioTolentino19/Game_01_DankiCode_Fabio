...
import com.tolentsgames.entities.*;
...
public class World {
			...
			for(int xx = 0; xx < map.getWidth(); xx++) {
				for(int yy = 0; yy < map.getHeight(); yy++) {
					...
					if(pixelAtual == 0xFF000000) {
						...
						// Flor
						tiles[xx + (yy * WIDTH)] = new FloorTile(xx*16, yy*16, Entity.FLOR_EN);
					} else if(pixelAtual == 0xFFFF006E) {
						// BigBoss
						tiles[xx + (yy * WIDTH)] = new FloorTile(xx*16, yy*16, Entity.BIGBOSS_EN);
					}
				}
			}
		} catch (IOException e) {
	...
	public static void generateParticles(int amount, int x, int y, int pColor) {
		for(int i = 0; i < amount; i++ ) {
			if(pColor == 0)
				Game.entities.add(new Particle(x+(TILE_SIZE/2), y+(TILE_SIZE/2), 1, 1, null, pColor));			
			if(pColor == 1) {
				Sound.explosionWall.play(Sound.effectsVolume);
				Game.entities.add(new Particle(x, y, 1, 1, null, pColor));
			}
		}
	}
	...
	public static void restartGame(String level) {
		...
		Game.minimapaShow = new boolean[Game.minimapaPixels.length];
		return;	
	}
	...
	public void render(Graphics g) {
		...
		for(int xx = xstart; xx <= xfinal; xx++) {
			for(int yy = ystart; yy <= yfinal; yy++) {
				...
				if(Tile.getTileSprite(tile) == Entity.BIGBOSS_EN && Game.gameState == "NORMAL")
					tile.setTileSprite(Tile.TILE_FLOOR);
				tile.render(g);
			}
		}
	}
	...
	public static void renderMiniMap() {
		...
		if(fullMap && !premioMap) {
			//Aqui preciso startar o priomfullmap System.out.println("Premio Full Map !!!");
			premioMap = true;
		}
		...
		Game.minimapaPixels[xPlayer + (yPlayer*WIDTH)] = 0xFF0000FF;
		...
		for(int i=0; i < Game.enemies.size(); i++) {
			...
			Game.minimapaPixels[xEnemy + (yEnemy*WIDTH)] = 0xFFFF6A00;
